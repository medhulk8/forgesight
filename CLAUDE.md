...
**Current: §13 steps 11–12 RAN. Step 11 full SFT done (single T4, fp16, max_pixels 384, 1 epoch/280 steps, loss 9.47→~1.4, adapter saved).** In-loop eval OOM'd (per_device_eval_batch_size default 8 → 8 val imgs through vision SDPA = 8.41GiB) → ran with eval_strategy="no", save at end. **Step 12 ForgeBench = class-collapse (NOT a bug): F1=0, 0 TP, both FT & baseline all-`tampered:false` with verbatim clean target "No inconsistencies detected." → adapter IS working (learned the constant clean string), model underfit the positive class.** Root cause: clean target = fixed constant (easy attractor) vs tampered = unique box each (hard); 1 epoch too few. splice+copy_move also missed → underfit-primary, not pure resolution. **DO NOT post F1=0 (artifact).**
**FIX QUEUED (quality run):** sft.yaml now num_train_epochs=3, model.py MAX_PIXELS 384→512 (both train+infer read same constant), eval_strategy="no"+save_strategy=epoch. Re-run ForgeBench → real numbers → fill README Results. Escalate epochs/res only if still collapsed. 122 tests green.
**Shipped:** README technical rewrite + MIT LICENSE (committed), resume bullets (method-only, no fabricated metrics, submitted). Kaggle drift fixed: sys.path shadow (cell 3), bnb unpinned (0.49.2), dataset mount /kaggle/input/datasets/<owner>/<slug>, storage=JPEG 247MB.

Earlier: §13 steps 8–10 code written. Persistence switched to **JSONL + PNGs** (not Arrow — version-robust; load via `load_dataset('json')`) — verified end-to-end on M3 (load + collator + span==target). Written: `requirements-kaggle.txt` (NO torch — use Kaggle CUDA build), `model.py` (4-bit NF4 + LoRA + sdpa), `train_sft.py` (SFTTrainer, `--overfit 8` mode), `configs/sft.yaml`, `notebooks/forgesight_kaggle.ipynb`, `KAGGLE_UPLOAD.md`. 113 tests green.
**Blocked on user (Kaggle):** upload `data/processed/` as private `forgesight-data` (~2.1 GB), GH_PAT in Secrets, run notebook → step-9 (4-bit load) + step-10 (overfit-8 loss→0, reproduces 8 JSONs) gates. I cannot run GPU/bitsandbytes on M3 (D10).
Dataset `data/processed/` (gitignored): 2964 ex (train 2244 / val 360 / test 360), doc-split 561/120/120, train 50/50, 0 leaks.
