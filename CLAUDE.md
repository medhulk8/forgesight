...
## Current stage
Stages §13 1–6 **done**. Step 6: `data/conversation.py` — `build_messages(record, data_root, include_target)` → Qwen2-VL chat messages (pure/disk-agnostic; image as `file://` URI not PIL; assistant target = `schema.to_target_json`). Real `Qwen2VLProcessor.apply_chat_template` render verified on M3/py3.14. 108 tests green. Next: §13 step 7 — `collator.py` (THE crux) + `scripts/smoke_collator.py`.
Verified token ids (Qwen2-VL-2B-Instruct): im_start=151644 im_end=151645 image_pad=151655 box_start=151648 box_end=151649; `assistant\n`=[77091,198] (collator header n_tail=2); box tokens survive tokenize→decode. Dataset in `data/processed/` (gitignored): 2964 ex (train 2244 / val 360 / test 360), doc-split 561/120/120, train 50/50, 0 leaks.
