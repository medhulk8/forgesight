...
**Current: §13 step 9 PASSED (4-bit load, LoRA 0.83%). Step 10 (overfit-8) pipeline PROVEN after fixing multi-GPU generate bug (device_map now single-GPU {'':0}) — digit_swap detects+boxes, clean correct. OPEN: recompress_ghost signal destroyed by JPEG-downscale storage (undetectable) — Gemini decision pending (store ghost lossless / drop / rework) before step 11.** Kaggle drift fixed: sys.path shadow (cell 3), bnb unpinned (0.49.2), dataset mount /kaggle/input/datasets/<owner>/<slug>, storage=JPEG 247MB.
